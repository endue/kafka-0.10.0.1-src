 ...
 */
case class LogOffsetMetadata(messageOffset: Long,// 下一条消息的偏移量，也就是LEO
                             segmentBaseOffset: Long = LogOffsetMetadata.UnknownSegBaseOffset,// 当前LogSegment的起始偏移量
                             relativePositionInSegment: Int = LogOffsetMetadata.UnknownFilePosition) {// 当前LogSegment的相对物理位置
